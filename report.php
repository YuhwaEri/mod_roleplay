<?php
    ...
    $url = new moodle_url('/mod/roleplay/report.php', array('id'=>$id));
    if ($download !== '') {
    ...
    require_capability('mod/roleplay:readresponses', $context);
    ...
    $strresponses = get_string("responses", "roleplay");
    ...
    if (data_submitted() && has_capability('mod/roleplay:deleteresponses', $context) && confirm_sesskey()) {
        if ($action === 'delete') {
    ...
    if (!$download) {
        ...
        $PAGE->set_title(format_string($roleplay->name).": $strresponses");
        ...
        if ($groupmode) {
            ...
            groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/roleplay/report.php?id='.$id);
        }
    } else {
        $groupmode = groups_get_activity_groupmode($cm);
    ...
    /// Calculate file name
        $filename = clean_filename("$course->shortname ".strip_tags(format_string($roleplay->name,true))).'.ods';
    /// Creating a workbook
    ...
    /// Print names of all the fields
        ...
        $myxls->write_string(0, $i++, get_string("roleplay", "roleplay"));

        // Generate the data for the body of the spreadsheet.
        $row = 1;
        if ($users) {
            foreach ($users as $option => $userid) {
                $option_text = roleplay_get_option_text($roleplay, $option);
    ...
    if ($download == "xls" && has_capability('mod/roleplay:downloadresponses', $context)) {
        require_once("$CFG->libdir/excellib.class.php");

    /// Calculate file name
        $filename = clean_filename("$course->shortname ".strip_tags(format_string($roleplay->name,true))).'.xls';
    /// Creating a workbook
    ...
    // print text file
    if ($download == "txt" && has_capability('mod/roleplay:downloadresponses', $context)) {
        $filename = clean_filename("$course->shortname ".strip_tags(format_string($roleplay->name,true))).'.txt';
        ...
        echo get_string("roleplay","roleplay"). "\n";
